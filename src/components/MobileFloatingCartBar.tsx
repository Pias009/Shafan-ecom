...
export function MobileFloatingCartBar() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="relative bg-[#073029]/95 backdrop-blur-2xl border border-emerald-400/35 rounded-2xl p-2.5 shadow-[0_16px_36px_rgba(0,0,0,0.35)] text-white overflow-hidden">
          {/* Subtle glowing ambient back-light */}
          <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-48 h-12 bg-emerald-400/20 blur-xl pointer-events-none" />
          ...
          <div className="mb-2">
            <div className="flex items-center justify-between text-[9.5px] font-bold tracking-tight mb-1">
              <span className="flex items-center gap-1 text-emerald-200">
                <Truck size={11} className="text-emerald-300" />
                {unlockedFree ? (
              ...
              <span className="text-[9px] text-emerald-300 font-bold">{progressPercent}%</span>
            </div>
            {/* Progress bar track */}
            <div className="w-full h-1 bg-black/30 rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-emerald-400 via-teal-300 to-amber-300 rounded-full transition-all duration-500 ease-out"
          ...
          <div className="flex items-center justify-between gap-2.5">
            ...
            >
              <div className="relative w-9 h-9 rounded-xl bg-white/15 border border-white/20 flex items-center justify-center shrink-0 shadow-inner">
                ...
                <span className="absolute -top-1 -right-1 w-4.5 h-4.5 bg-rose-500 text-white text-[9px] font-black rounded-full flex items-center justify-center border border-[#073029] shadow-xs">
                  {cartCount}
                </span>
              </div>
              <div className="flex flex-col leading-none">
                <span className="text-[9px] text-emerald-200/90 uppercase font-black tracking-wider mb-0.5">
                  Subtotal
                </span>
                <Price
                  amount={subtotal}
            ...
            <Link
              href="/cart"
              className="relative group inline-flex items-center justify-center gap-1.5 bg-gradient-to-r from-emerald-400 via-teal-400 to-emerald-500 text-[#073029] px-4 py-2 rounded-xl font-black text-xs uppercase tracking-wider shadow-[0_4px_16px_rgba(16,185,129,0.4)] hover:brightness-110 active:scale-95 transition-all shrink-0"
            >
              <span>Checkout</span>
