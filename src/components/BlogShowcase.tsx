...
export function BlogShowcase() {
  ...
  useEffect(() => {
    fetch("/api/blog")
      .then((r) => r.json())
      .then((data) => {
        if (Array.isArray(data)) setPosts(data.slice(0, 6));
      })
      .catch(() => {});
  }, []);
    ...
    <section className="mx-auto max-w-7xl px-6 pt-20 pb-4">
      ...
      <div className="relative">
        <div 
          ...
          className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide -mx-4 px-4 md:grid md:grid-cols-1 md:overflow-visible md:snap-none lg:grid-cols-2 xl:grid-cols-3 md:!overflow-visible"
        >
          {posts.map((post, i) => (
            <Link
              key={post.id}
              href={`/blog/${post.slug}`}
              className="group block snap-start w-[280px] md:w-auto flex-shrink-0"
            >
              <article className="glass-panel-heavy h-full min-h-[200px] p-5 rounded-2xl hover:shadow-xl hover:shadow-black/15 hover:translate-y-[-2px] transition-all duration-300 flex flex-col h-full">
        ...
        <button
          ...
          className={`absolute top-1/2 -left-2 md:-left-4 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow-lg border border-black/10 flex items-center justify-center transition-all ${canScrollLeft ? 'hover:bg-black hover:text-white' : 'opacity-30 cursor-not-allowed'}`}
        >
          <ChevronLeft size={18} />
        </button>
        <button
          ...
          className={`absolute top-1/2 -right-2 md:-right-4 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow-lg border border-black/10 flex items-center justify-center transition-all ${canScrollRight ? 'hover:bg-black hover:text-white' : 'opacity-30 cursor-not-allowed'}`}
        >
      ...
      <div className="flex md:hidden items-center justify-center gap-2 mt-4">
        ...
        <div className="flex gap-1.5">
          {posts.slice(0, Math.min(posts.length, 4)).map((_, i) => (
            <div
              key={i}
