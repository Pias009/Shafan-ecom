...
export function FreshFromShelfTable({ initialProducts }: { initialProducts: Product[] }) {
  const [search, setSearch] = useState("");
  const [activeTab, setActiveTab] = useState<"newest" | "shelf" | "available">("newest");
  ...
  const getDisplayProducts = () => {
    if (activeTab === "newest") return newestProducts;
    if (activeTab === "shelf") return shelfProductsList;
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex bg-white border border-black/5 rounded-full p-1">
          ...
          >
            <Sparkles className="w-4 h-4 inline mr-1" /> Newest ({Math.min(newestProducts.length, 20)})
          </button>
          <button
          ...
          >
            <Package className="w-4 h-4 inline mr-1" /> On Shelf ({shelfProductsList.length})
          </button>
          <button
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-black/5">
            {getDisplayProducts().map((product) => {
              const isOnShelf = product.freshFromShelf === true;
                ...
                <tr key={product.id} className="hover:bg-black/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="font-black text-sm">
                      AED {getDisplayPrice(product).toFixed(2)}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    {isOnShelf ? (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-100 text-emerald-800 text-xs font-black uppercase rounded-full">
                        <Package className="w-3 h-3" /> On Shelf
                      </span>
                    ) : (
                  ...
                  <td className="px-6 py-4 text-right">
                    ...
                    >
                      {isUpdating ? (
                        ...
                        <>
                          <Package className="w-4 h-4" /> Remove
                        </>
                      ) : (
                        <>
                          <Package className="w-4 h-4" /> Add to Shelf
                        </>
                      )}
                    </button>
                  </td>
        ...
        {getDisplayProducts().length === 0 && (
          <div className="py-20 text-center">
            <div className="text-6xl mb-4 opacity-30">
              {activeTab === "newest" ? "✨" : activeTab === "shelf" ? "📦" : "📋"}
            </div>
