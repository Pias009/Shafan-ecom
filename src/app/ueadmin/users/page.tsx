    ...
    <div className="space-y-8 pb-20 max-w-7xl mx-auto px-4 md:px-0">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-sm font-bold text-black/30 uppercase tracking-[0.2em] leading-none">
        ...
        <div className="flex items-center gap-3">
            <div className="relative group">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-black/20 group-focus-within:text-black transition-colors" size={16} />
                <input 
                    type="text" 
            ...
            <button className="h-12 w-12 bg-black/5 rounded-2xl flex items-center justify-center text-black/40 hover:bg-black/10 transition-all">
                <Filter size={18} />
            </button>
        </div>
      </div>
      ...
      <div className="glass-panel-heavy rounded-[2.5rem] border border-black/5 bg-white shadow-2xl shadow-black/5 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-black/5 bg-black/[0.02] text-[10px] font-black uppercase tracking-widest text-black/30">
                ...
                <tr key={u.id} className="group hover:bg-black/[0.01] transition-all">
                  <td className="px-8 py-6">
                    <Link href={`/ueadmin/users/${u.id}`} className="flex items-center gap-4 group/item">
                      <div className="h-12 w-12 rounded-2xl bg-black/5 flex items-center justify-center text-black/20 group-hover:bg-black group-hover:text-white transition-all duration-500 shadow-lg shadow-transparent group-hover:shadow-black/20">
                        <User size={20} />
                      </div>
                      <div>
                        <div className="font-bold text-black text-base group-hover/item:underline">{u.name || 'Anonymous User'}</div>
                        <div className="text-[10px] font-bold text-black/40 flex items-center gap-1.5 uppercase tracking-wider mt-0.5">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2 text-black/50 text-xs font-semibold">
                  ...
                  <td className="px-8 py-6 text-right">
                    <Link 
                      ...
                      className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-black/5 text-black/20 hover:bg-black hover:text-white transition-all shadow-lg shadow-transparent hover:shadow-black/10"
                    >
