...
export async function POST({ params, request }: { params: { id: string }, request: Request }) {
  ...
  try {
    ...
    let shipment = await (prisma as any).shipment.findUnique({ where: { orderId: params.id } });
    if (!shipment) {
      shipment = await (prisma as any).shipment.create({ data: { orderId: params.id, courier: parsed.data.courier ?? '', trackingCode: parsed.data.trackingCode ?? '', trackingUrl: parsed.data.trackingUrl ?? '', status: 'Created' } });
    } else {
